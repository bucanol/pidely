...
const clientTools = [
  { type: "function" as const, function: { name: "get_menu", description: "Menú completo con precios y descripciones.", parameters: { type: "object", properties: {}, required: [] } } },
  { type: "function" as const, function: { name: "get_my_bill", description: "Cuenta actual de la mesa.", parameters: { type: "object", properties: {}, required: [] } } },
...
const kitchenTools = [
  ...
  { type: "function" as const, function: { name: "get_menu_with_details", description: "Recetas e ingredientes del menú.", parameters: { type: "object", properties: {}, required: [] } } },
];
...
const ownerTools = [
  ...waiterTools,
  { type: "function" as const, function: { name: "get_analytics", description: "Estadísticas completas de ventas: hoy, mes, total y top productos.", parameters: { type: "object", properties: {}, required: [] } } },
  { type: "function" as const, function: { name: "get_menu", description: "Menú completo del restaurante.", parameters: { type: "object", properties: {}, required: [] } } },
  { type: "function" as const, function: { name: "get_active_orders", description: "Órdenes activas del restaurante.", parameters: { type: "object", properties: {}, required: [] } } },
  ...
  try {
    switch (name) {
      ...
      case "place_order": {
        ...
        return JSON.stringify({ success: true, message: "Orden enviada a cocina." });
      }
      case "call_waiter": {
        if (!tableId) return JSON.stringify({ error: "Mesa no identificada" });
...
- SIEMPRE usa las herramientas disponibles para consultar datos reales
...
- Consulta get_menu antes de sugerir cualquier platillo
- Detecta alergias y filtra sugerencias del menú real
- Permite pedir, ver cuenta, llamar mesero
- Si piden algo fuera del menú: "Lo sentimos, no contamos con ese platillo."
...
- Usa get_active_orders para ver órdenes
- Usa get_menu_with_details para ingredientes y recetas
- Sé directo y rápido`;
...
- Usa get_tables_status para ver mesas
- Usa get_table_bill para cuentas
- Usa get_waiter_calls para llamadas pendientes`;

    case "owner":
      return `${base}

ROL: Asistente ejecutivo del dueño de "${restaurantName}".
- Usa get_analytics para ventas y estadísticas
- Usa get_tables_status para estado del restaurante
- Usa get_tickets_history para historial de pagos
- Da reportes ejecutivos claros con números reales
- Sugiere promociones basadas en datos reales`;

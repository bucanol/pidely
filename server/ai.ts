...
// --- TOOLS (ESTRUCTURA OPTIMIZADA) ---
const clientTools = [
  { type: "function" as const, function: { name: "get_menu", description: "Menú con precios y descripciones.", parameters: { type: "object", properties: {}, required: [] } } },
      ...
      case "get_menu": {
        ...
        return JSON.stringify(categories.map(cat => ({
          ...
          platillos: products.filter(p => p.categoryId === cat.id).map(p => ({
            id: p.id,
            nombre: p.name,
            descripcion: p.description || "No especificada", // REPARADO: Ahora la IA sabe qué lleva el plato
...
MENÚ REAL DISPONIBLE (SÓLO PUEDES RESPONDER CON ESTO):
${menuData}

REGLAS DE ORO:
1. SÓLO PUEDES SUGERIR O VENDER LO QUE ESTÁ EN EL MENÚ REAL.
...
4. CONFIRMACIÓN: Tras usar una herramienta (pedir, llamar mesero), confirma amablemente que la acción se realizó. TÚ eres el asistente, nunca pidas cosas para ti.`;
...
function selectModel(message: string, role: string): string {
  ...
  const isSensitive = /taco|comida|hambre|menu|precio|platillo|orden|pide|lleva|ingrediente|que es|contiene|alergia/i.test(message);
...
async function runAILoop(messages: any[], tools: any[], restaurantId: string, role: string, tableId?: string, slug?: string, modelUsed: string = POWER_MODEL): Promise<string> {
  ...
  while (assistantMessage.tool_calls && assistantMessage.tool_calls.length > 0) {
    ...
    messages.push({ role: "system", content: "ACCIÓN COMPLETADA. Confirma al usuario de forma breve. No pidas nada adicional tú mismo." });
